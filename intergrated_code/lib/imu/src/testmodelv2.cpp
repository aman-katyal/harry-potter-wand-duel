...
// Struct with classification decision
typedef struct {
...
// ---------------------------------------------------------------------------
// Core 1 entrypoint for multicore setup
//  - Runs IMU + Edge Impulse
//  - Only sends a spell to core 0 when it sees the same
//    non-idle class 5 times in a row.
// ---------------------------------------------------------------------------

extern "C" void imu_core1_entry(void) {
    const int REQUIRED_CONSISTENT_DETECTIONS = 5;

    int last_class_idx = -1;
    int repeat_count   = 0;

    while (true) {
        // One IMU + inference step (includes init on first call)
        SpellDecision decision = get_spell_decision();

        // If we didn't get a valid detection this iteration, just loop
        if (!decision.detected || decision.label == NULL) {
            // Reset the streak when confidence drops or nothing detected
            last_class_idx = -1;
            repeat_count   = 0;
            tight_loop_contents();
            continue;
        ...
        // Map label string -> class index as used by Edge Impulse
        int class_idx = -1;
        for (uint16_t i = 0; i < EI_CLASSIFIER_LABEL_COUNT; i++) {
            if (strcmp(decision.label, ei_classifier_inferencing_categories[i]) == 0) {
                class_idx = (int)i;
                break;
            }
        }
        ...
        // OPTIONAL: treat "idle" as "no spell"
        // (Assumes your labels are ["aguamenti", "idle", "stupefy"]).
        // Adjust the string if your idle label is named differently.
        if (strcmp(decision.label, "idle") == 0) {
            last_class_idx = -1;
            repeat_count   = 0;
            tight_loop_contents();
            continue;
        ...
        // Check for N identical detections in a row
        ...
        // When we’ve seen the same class enough times, send it once to core 0
        if (repeat_count >= REQUIRED_CONSISTENT_DETECTIONS) {
            printf("[Core1] Stable gesture detected: %s (idx=%d, conf=%.2f)\n",
                   decision.label, class_idx, decision.confidence);

            // Send the class index to Core 0.
            // Core 0 will map this index -> spell number.
            multicore_fifo_push_blocking((uint32_t)class_idx);

            // Reset streak so we need 5 fresh detections before sending again
            last_class_idx = -1;
            repeat_count   = 0;

            // Small pause to avoid spamming multiple times from the same motion
            sleep_ms(100);
        }

        // Let other stuff run, but don't delay too much:
        // timing is already mostly governed by get_spell_decision().
        tight_loop_contents();
    }
}

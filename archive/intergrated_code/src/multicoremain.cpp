...
// Gesture-recognition entry point that runs on core 1.
// Implemented in your IMU / Edge Impulse file (e.g. testmodelv2.cpp).
...
#define BTN_B 26  // Spell 2  (A+B = Spell 3)
...
// GAME CONFIGURATION
// ===================================
#define MY_PLAYER_ID        1  // Change to 2 on the other wand
#define OPPONENT_PLAYER_ID  2
#define PACKET_REPEATS      3

#define RESPAWN_DELAY_MS 3000  // “Dead” time after losing
...
void init_buttons() {
    // Active High (Press = 1), pull-down resistors enabled
...
void process_hit_effects(uint8_t spell_num) {
    ...
    switch (spell_num) {
        case 1:
            ...
            anim_id = 3; // Spiral in controller.c
            damage  = 1;
            break;
        case 2:
    ...
    // 3. Blocking animation (stun duration)
    controller(anim_id, LED_WIDTH, LED_HEIGHT);

    // 4. Redraw health bar after animation
    hb_draw();
}

int main() {
    stdio_init_all();
    sleep_ms(2000); // Allow USB serial to come up etc.

    printf("\n=== Wand Duel (RP2350, Core0 Game + Core1 IMU) ===\n");

    // -------------------------------------------------
    // Launch IMU / Gesture Recognition on Core 1
    // -------------------------------------------------
    multicore_launch_core1(imu_core1_entry);
    multicore_fifo_drain(); // Clear any stale FIFO contents

    // -------------------------------------------------
    // Core 0: Initialize Hardware for game logic
    // -------------------------------------------------
    init_buttons();
    ...
    while (true) {
        ...
        if (!is_dead && !currently_casting && ir_emitter_done()) {
            if (multicore_fifo_rvalid()) {
                ...
                // Map IMU class index -> spell
                // Adjust this mapping to your Edge Impulse label order.
                int spell_to_cast = 0;
                switch (class_idx) {
                    case 0: // aguamenti
                        spell_to_cast = 1;
                    ...
                    case 2: // stupefy
                        spell_to_cast = 2;
                        break;
                    case 3: // extra gesture (if you add one)
                        spell_to_cast = 3;
                        break;
                    default:
                        spell_to_cast = 0;
        ...
        // ------------------------------------------
        // 4b. BUTTON CASTING (optional fallback)
        // ------------------------------------------
        if (!is_dead && !currently_casting && ir_emitter_done()) {

/**
 * @file main.c
 * @brief Dual-Core Wand Duel - Dynamic Thresholds Fixed
 */
...
int main() {
    ...
    while (true) {
        ...
        if (multicore_fifo_rvalid()) {
            ...
            // --- DYNAMIC THRESHOLDS ---
            float anomaly_limit = 100.0f; // Default loose
            ...
            // FIXED: Use the dynamic variable `anomaly_limit` here
            ...
            if (spell_score >= SPELL_TRIGGER_TARGET) {
                // FIXED: Cast to (int) to resolve warning
                printf(">>> CAST SPELL %d! <<<\n", (int)candidate_spell);
                ...
                // Map Logic ID -> LED Animation ID
                if (candidate_spell == SPELL_LOGIC_AGUAMENTI) {
                    controller(2, MATRIX_WIDTH, MATRIX_HEIGHT); // Blue Firework
                } 
                else if (candidate_spell == SPELL_LOGIC_STUPEFY) {
                    controller(1, MATRIX_WIDTH, MATRIX_HEIGHT); // Red Firework
                }
